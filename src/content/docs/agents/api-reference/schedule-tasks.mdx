...
For tasks that need to run at fixed intervals (sub-minute precision or arbitrary durations), use `this.scheduleEvery()`:
...
- **Minimum granularity:** 1 second (vs. 1 minute for cron)
- **Arbitrary intervals:** Yes (vs. must fit cron pattern)
- **Overlap prevention:** Built-in (if callback is still running when next execution is due, it is skipped)
- **Error resilience:** If callback throws an error, the interval continues
