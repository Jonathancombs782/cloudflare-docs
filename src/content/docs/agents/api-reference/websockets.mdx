...
### Callable methods over WebSocket

Mark methods with `@callable()` to expose them for RPC (Remote Procedure Call) from WebSocket clients:
...
// Using call() with timeout
const result = await agent.call("greet", ["World"], { timeout: 5000 });

// Streaming responses
await agent.call("generateText", [prompt], {
  stream: {
    onChunk: (chunk) => appendToOutput(chunk),
    onDone: (finalValue) => console.log("Complete"),
...
Callable methods support:
- **Timeouts**: Set a timeout in milliseconds with the `timeout` option
- **Streaming responses**: Send data incrementally with `StreamingResponse`
- **Error handling**: Gracefully handle errors with `stream.error(message)`
- **Introspection**: Get callable method metadata with `agent.getCallableMethods()`
